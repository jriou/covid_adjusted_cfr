...
data_bavaria=transmute(data_germany,date=dmy(as.character(date)),new_cases=cases_bavaria,new_deaths=c(NA,diff(deaths_bavaria))) %>%
  filter(date>=ymd(day_data),date<=ymd(day_max))

incidence_cases=data_bavaria$new_cases
incidence_deaths=data_bavaria$new_deaths
p_underreport_cases=sum(incidence_cases)/36538
p_underreport_deaths=1

plot(data_bavaria$date,incidence_cases,type="l",col="firebrick")
plot(data_bavaria$date,incidence_deaths,type="l",col="seagreen")
