...
# Load posterior samples (first extract chains and save them in run_model13LO_LOBly.R)
l = load("posterior_samples/model13LO_2020-03-25.Rdata")


sum(D_S_model13LOB$incidence_deaths)/sum(D_S_model13LOB$incidence_cases) *100
## total symptomatic cases
sum(D_S_model13LOB$incidence_cases)
print(S_model13LOB,"predicted_total_overall_symptomatic_cases")
summary(S_model13LOB,"predicted_total_overall_symptomatic_cases")[[1]] / sum(D_S_model13LOB$incidence_cases) 
## reporting rate by age
print(S_model13LOB,"rho")
## proportion symptomatic (data)
print(S_model13LOB,"psi")
## total cases
print(S_model13LOB,"predicted_total_overall_all_cases")
## total cases
print(S_model13LOB,"predicted_total_overall_deaths_delay")
## reported deaths
sum(D_S_model13LOB$incidence_deaths) 
## CFR among symptomatics
print(S_model13LOB,"cfr_D_symptomatic",digits_summary = 4)
## CFR among all
print(S_model13LOB,"cfr_D_all",digits_summary = 4)
## CFR among all by age
summary(S_model13LOB,"epsilon",digits_summary = 4)[[1]][,c(6,4,8)]*100

# Figure 1: data ----
source("data/lombardy/data_management_lombardy.R")

itcases = data.frame(cases=D_S_model13LOB$incidence_cases,
                     time=1:D_S_model13LOB$D) %>%
  mutate(date=day_data+time)
itdeaths = data.frame(deaths=D_S_model13LOB$incidence_deaths,
                     time=1:D_S_model13LOB$D) %>%
  mutate(date=day_data+time)
...
dd2 = data.frame(age_class=c("0-9","10-19","20-29","30-39","40-49","50-59","60-69","70-79","80+"),
                 pop = D_S_model13LOB$age_dist,
                 cases_tmax=D_S_model13LOB$agedistr_cases/sum(D_S_model13LOB$agedistr_cases),
                 mort_tmax=D_S_model13LOB$agedistr_deaths/sum(D_S_model13LOB$agedistr_deaths)) 
...
contacts = matrix(D_S_model13LOB$contact,nrow=9,byrow=T) %>%
  data.frame() %>%
  tbl_df()
...
ggsave(file=paste0(figpath,"data_lombardy.pdf"),width=10,height=5.5)


# Figure 2: model fit ----
D_S_model13LOB$tswitch = D_S_model13LOB$D

i1 = plot_incidence_cases(S_model13LOB,D_S_model13LOB,start_date=day_start,end_date=day_max+60,
                          col1=col.cases1,col2=col.cases2,col3=col.cases3)
i1
i2 = plot_total_cases(S_model13LOB,D_S_model13LOB,
                      col1=col.cases1,col2=col.cases2,col3=col.cases3)
i2
i3 = plot_agedist_cases(S_model13LOB,D_S_model13LOB,
                        col1=col.cases1,col2=col.cases2,col3=col.cases3)
i3

j1 = plot_incidence_deaths2(S_model13LOB,D_S_model13LOB,start_date=day_start,end_date=day_max+60,
                           col1=col.deaths1,col2=col.deaths2)
j1
j2 = plot_total_deaths(S_model13LOB,D_S_model13LOB,
                       col1=col.deaths1,col2=col.deaths3)
j2
j3 = plot_agedist_deaths(S_model13LOB,D_S_model13LOB,
                         col1=col.deaths1,col2=col.deaths3)
j3
...
ggsave(file=paste0(figpath,"modelfit_lombardy.pdf"),width=10,height=5)


# Figure 3: age specific CFR

k1 = plot_agedist_cfr(S_model13LOB,D_S_model13LOB,col1="tomato",col2=col.deaths2,col3=col.cases3)

k2 = plot_cfr(S_model13LOB,D_S_model13LOB,col1="tomato",col2=col.deaths2,col3=col.cases3)
...
ggsave(file=paste0(figpath,"cfr_lombardy.pdf"),width=9,height=3.8)
...
# supp table
summary(S_model13LOB,pars=c("beta","psi","pi"))[[1]][,c(6,4,8)] %>%
  as.data.frame() %>%
  tbl_df() %>%
...
rho = summary(S_model13LOB,pars="rho")[[1]][,c(6,4,8)] %>%
  as.data.frame() %>%
  tbl_df() %>%
...
epsilon = summary(S_model13LOB,pars="epsilon")[[1]][,c(6,4,8)] %>%
  as.data.frame() %>%
  tbl_df() %>%
  mutate(age_group=c("0-9","10-19","20-29","30-39","40-49","50-59","60-69","70-79","80+"),
