...
# Load posterior samples (first extract chains and save them in run_model13ES_ESBly.R)
l = load("posterior_samples/model13ES_2020-03-25.Rdata")


sum(D_S_model13ESB$incidence_deaths)/sum(D_S_model13ESB$incidence_cases) *100
## total symptomatic cases
sum(D_S_model13ESB$incidence_cases)
print(S_model13ESB,"predicted_total_overall_symptomatic_cases")
summary(S_model13ESB,"predicted_total_overall_symptomatic_cases")[[1]] / sum(D_S_model13ESB$incidence_cases) 
## reporting rate by age
print(S_model13ESB,"rho")
## proportion symptomatic (data)
print(S_model13ESB,"psi")
## total cases
print(S_model13ESB,"predicted_total_overall_all_cases")
## total cases
print(S_model13ESB,"predicted_total_overall_deaths_delay")
## reported deaths
sum(D_S_model13ESB$incidence_deaths) 
## CFR among symptomatics
print(S_model13ESB,"cfr_D_symptomatic",digits_summary = 4)
## CFR among all
print(S_model13ESB,"cfr_D_all",digits_summary = 4)
## CFR among all by age
summary(S_model13ESB,"epsilon",digits_summary = 4)[[1]][,c(6,4,8)]*100

# Figure 1: data ----
source("data/spain/data_management_spain.R")

itcases = data.frame(cases=D_S_model13ESB$incidence_cases,
                     time=1:D_S_model13ESB$D) %>%
  mutate(date=day_data+time)
itdeaths = data.frame(deaths=D_S_model13ESB$incidence_deaths,
                     time=1:D_S_model13ESB$D) %>%
  mutate(date=day_data+time)
...
dd2 = data.frame(age_class=c("0-9","10-19","20-29","30-39","40-49","50-59","60-69","70-79","80+"),
                 pop = D_S_model13ESB$age_dist,
                 cases_tmax=D_S_model13ESB$agedistr_cases/sum(D_S_model13ESB$agedistr_cases),
                 mort_tmax=D_S_model13ESB$agedistr_deaths/sum(D_S_model13ESB$agedistr_deaths)) 
g1 = select(dd2,age_class,pop,cases_tmax,mort_tmax) %>%
  gather("var","value",2:4) %>%
  mutate(var2=factor(var,levels=c("pop","cases_tmax","mort_tmax"),labels=c("Spanish population","Reported cases","Reported deaths"))) %>%
  ggplot() +
...
contacts = matrix(D_S_model13ESB$contact,nrow=9,byrow=T) %>%
  data.frame() %>%
  tbl_df()
...
ggarrange(h1,g1,h2,g2,nrow=2,ncol=2,widths=c(1.3,1),labels=LETTERS)
  ...
  ggsave(file=paste0(figpath,"data_spain.pdf"),width=10,height=5.5)


# Figure 2: model fit ----
D_S_model13ESB$tswitch = D_S_model13ESB$D

i1 = plot_incidence_cases(S_model13ESB,D_S_model13ESB,start_date=day_start,end_date=day_max+60,
                          col1=col.cases1,col2=col.cases2,col3=col.cases3)
i1
i2 = plot_total_cases(S_model13ESB,D_S_model13ESB,
                      col1=col.cases1,col2=col.cases2,col3=col.cases3)
i2
i3 = plot_agedist_cases(S_model13ESB,D_S_model13ESB,
                        col1=col.cases1,col2=col.cases2,col3=col.cases3)
i3

j1 = plot_incidence_deaths2(S_model13ESB,D_S_model13ESB,start_date=day_start,end_date=day_max+60,
                           col1=col.deaths1,col2=col.deaths2)
j1
j2 = plot_total_deaths(S_model13ESB,D_S_model13ESB,
                       col1=col.deaths1,col2=col.deaths3)
j2
j3 = plot_agedist_deaths(S_model13ESB,D_S_model13ESB,
                         col1=col.deaths1,col2=col.deaths3)
j3
...
ggsave(file=paste0(figpath,"modelfit_spain.pdf"),width=10,height=5)


# Figure 3: age specific CFR

k1 = plot_agedist_cfr(S_model13ESB,D_S_model13ESB,col1="tomato",col2=col.deaths2,col3=col.cases3)

k2 = plot_cfr(S_model13ESB,D_S_model13ESB,col1="tomato",col2=col.deaths2,col3=col.cases3)
...
ggsave(file=paste0(figpath,"cfr_spain.pdf"),width=9,height=3.8)
...
# supp table
summary(S_model13ESB,pars=c("beta","psi","pi"))[[1]][,c(6,4,8)] %>%
  as.data.frame() %>%
  tbl_df() %>%
...
rho = summary(S_model13ESB,pars="rho")[[1]][,c(6,4,8)] %>%
  as.data.frame() %>%
  tbl_df() %>%
...
epsilon = summary(S_model13ESB,pars="epsilon")[[1]][,c(6,4,8)] %>%
  as.data.frame() %>%
  tbl_df() %>%
  mutate(age_group=c("0-9","10-19","20-29","30-39","40-49","50-59","60-69","70-79","80+"),
