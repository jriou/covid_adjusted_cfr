...
parameters{
  ...
  real kappa; // change in reporting due to comobidity
...
transformed parameters {
  ...
  rho_com[K] = 1.0; // probability of reporting given age 80+ and comorbidity = 1
  rho = inv_logit(logit(rho_com)-kappa); // probability of reporting without comorbidity
  rho_global = rho .* (1.0 - comorbidity_dist) + rho_com .* comorbidity_dist; // probability of reporting overall
  ...
  y = integrate_ode_bdf(
    ...
    for(i in t_data:S){
      output_incidence_cases[i-t_data+1] = sum(comp_diffC[i].*rho_global);
      output_incidence_deaths[i-t_data+1] = sum(comp_diffM[i]);
    }
    output_agedistr_cases = (comp_C[S,].*rho_global) ./ sum(comp_C[S,].*rho_global);
    output_agedistr_deaths = (comp_M[S,]) ./ sum(comp_M[S,]);
    output_prop_cases_with_comorbidity = sum(comp_D[S].*rho_com)/sum(comp_C[S].*rho_com);
...
generated quantities{
  real avg_rho = sum(age_dist .* rho_global);
  real beta2 = beta*eta;
  ...
  real predicted_p_deaths_comorbidity;
  real predicted_p_cases_comorbidity;
  ...
  for(i in 1:S){
    predicted_reported_incidence_symptomatic_cases[i] = neg_binomial_2_rng(sum(comp_diffC[i].*rho_global), sum(comp_diffC[i].*rho_global)/phi[1]);
  ...
  predicted_p_cases_comorbidity = (binomial_rng(n_cases_comorbidity, output_prop_cases_with_comorbidity) + 1e-9)/n_cases_comorbidity;
  predicted_p_deaths_comorbidity = (binomial_rng(n_deaths_comorbidity, output_prop_deaths_with_comorbidity) + 1e-9)/n_deaths_comorbidity;
  for(i in 1:S) {
    ...
    predicted_comp_overall_diffC[i] = to_vector(predicted_comp_reported_diffC[i]) ./ rho_global;
    predicted_comp_overall_diffA[i] = to_vector(predicted_comp_reported_diffC[i]) ./ rho_global * (1-psi) / psi;
  }
