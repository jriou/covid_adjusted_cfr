functions {
  ...
  int[] x_i
    ...
    real mu_1; // infectious period
    real mu_2; // time until recovery or death
    ...
    for (k in 1:K) {
      ...
      // I
      dydt[2*K+k] = tau* p_tmax * y[K+k] - mu_1 * y[2*K+k];
      // J
      dydt[3*K+k] = mu_1 * y[2*K+k] - mu_2 * y[3*K+k];
      // R
      dydt[4*K+k] = mu_2 * (1.0 - epsilon[k]) * y[3*K+k];
      // C
      dydt[5*K+k] = tau* p_tmax * y[K+k];
      // M
      dydt[6*K+k] = mu_2 * epsilon[k] * y[3*K+k];
    }
    return(dydt);
  }
}
...
transformed data {
  real x_r[5] = {tmax2,tswitch,p_incubation,p_infectious,p_recovery_or_death};
  int x_i[1] = {K};
}
...
transformed parameters {
  ...
  real theta[3+K-1]; // vector of parameters
  ...
  // change of format for integrate_ode_rk45
  theta[1:2] = {beta,eta};
  theta[3:(3+K-1)] = to_array_1d(epsilon);
  for(k in 1:K){
