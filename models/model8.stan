...
parameters{
  ...
  real<lower=0> isolation_to_death; // time from isolation to death
...
transformed parameters {
  ...
  vector[K] rho_K;
  real xi = xi_raw+0.5;
  real delta = 1.0/isolation_to_death;
  // change of format for integrate_ode_rk45
  real theta[6+K-1]; // vector of parameters
  ...
  // transformed parameters
  for(i in 1:(K-1)){
    rho_K[i]=rho[i]*p_symptomatic;
  }
  rho_K[K]=p_symptomatic;
  // change of format for integrate_ode_rk45
  theta[1:5] = {beta,eta,delta,xi,nu};
  theta[6:(6+K-1)] = to_array_1d(epsilon);
  for(k in 1:K){
...
model {
  ...
  isolation_to_death ~ gamma(p_isolation_to_death[1],p_isolation_to_death[2]);
